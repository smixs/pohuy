...
Подробная настройка команды, TUI и источников стиля: [расширение для Pi](./extensions/README.md).

## **Включить:** `/pohuy`, «включи похуй-режим» или «отвечай матом». **Выключить:** `/pohuy normal`, «нормальный режим» или «хватит материться».

Случайный мат, цитата или раздражение режим не включают: нужен явный opt-in.
...
Три уровня. Переключение: `/pohuy <уровень>`. В skill-команде выбор держится до
конца текущей сессии; Pi хранит свой калибр между перезапусками.

| Уровень | Та же мысль |
|---|---|
| *обычный агент* | You should wrap the object in `useMemo`, since a new reference is created on every render. |
| `lite` *(default)* | Оберни в `useMemo`: новый ref на каждый рендер, вот оно и перерисовывается. Отъебётся. |
| `full` | Хуйня вопрос: inline-объект = новый ref = ре-рендер. `useMemo` — и не еби мозг. |
...
Runtime хранит правила тона, калибровки серьёзности и safety-границы в компактном
семантическом ядре. Полные словарь, сцены и онтология остаются в `references/`
как опциональный материал и автоматически в контекст не загружаются.
...
- Словарь на 70+ идиом с инженерным маппингом — опциональный боекомплект
